<template>
  <div class="home-blog" :class="recoShow?'reco-show': 'reco-hide'">
    <div class="hero" :style="{background: `url(${$frontmatter.bgImage[Math.round(Math.random()*($frontmatter.bgImage.length-1))] ? $withBase($frontmatter.bgImage[Math.round(Math.random()*($frontmatter.bgImage.length-1))]) : require('../images/home-bg.jpg')}) center/cover no-repeat`, ...bgImageStyle}">
